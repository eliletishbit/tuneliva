  ...
  return (
    <div className="min-h-screen bg-[#060913] text-white selection:bg-indigo-500 selection:text-white pb-24">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-8 py-8 space-y-10">
        ...
        {/* ONGLET DE NAVIGATION SUPER ADMIN */}
        <div className="flex items-center gap-2 overflow-x-auto pb-2 border-b border-slate-800">
          ...
          <button
            type="button"
            onClick={() => setActiveAdminTab("tunnels")}
            className={`px-4 py-2.5 rounded-2xl text-xs font-bold transition-all flex items-center gap-2 cursor-pointer ${
              activeAdminTab === "tunnels"
                ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/30 font-black"
                : "bg-slate-900/60 text-slate-400 hover:text-white hover:bg-slate-900 border border-slate-800"
            }`}
          >
            <Layers className="w-3.5 h-3.5" />
            <span>🚀 Tunnels Créés ({metrics?.tunnels?.length || 0})</span>
          </button>
          ...
          >
            ...
            <span>👥 Utilisateurs ({metrics?.users?.length || 0})</span>
          </button>
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-2xl text-xs font-bold transition-all flex items-center gap-2 cursor-pointer ${
              activeAdminTab === "tiktok"
                ? "bg-gradient-to-r from-rose-500 via-pink-500 to-purple-600 text-white shadow-lg shadow-rose-500/30 font-black"
                : "bg-slate-900/60 text-slate-400 hover:text-white hover:bg-slate-900 border border-slate-800"
            }`}
          >
            <Smartphone className="w-3.5 h-3.5 text-rose-400" />
            <span>📱 Publication TikTok IA</span>
            <span className="px-1.5 py-0.5 rounded-full bg-rose-500/20 text-rose-300 text-[10px] font-mono font-bold">2/jour</span>
          </button>
        </div>

        {/* AFFICHAGE CONDITIONNEL TIKTOK */}
        {activeAdminTab === "tiktok" && (
